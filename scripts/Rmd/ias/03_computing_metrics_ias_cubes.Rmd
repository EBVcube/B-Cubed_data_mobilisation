...
```
We start loading all libraries needed in this notebook.
...
gbif_tax <- read.csv(here("input/data/ias_10/taxonomy/List87IAS_EU_match_gbif_allaccepted.csv"))
...
#### **Metric 1**: Calculate total number of occurrences at GBIF pixelwise

The function `total_occ` calculates the cumulative value of occurrences per pixel for each specie.
```{r}
...
ggplot() +
  ...
  scale_fill_viridis_c(name="Total occurrences") + # Optional: for better color scale
  theme_minimal() +
  labs(
...
#### **Metric 2 and 3**: Earliest and latest date of occurences at GBIF

The following function finds the earliest and latest date of records per species at pixel level across the whole data set.
...
In the following code chunck we compute metrics 2 and 3. Be aware that the last function argument specifies whether we calculate the earliest or latest date of record in GBIF.
...
Note that the `min_max_dates` function returns a list. The first element of the list is a raster containing the earliest(latest) date of the records. The second element is a table that will be used in the next section to calculate metrics 4 and 5.
```{r}
...
This code chunk is for plotting the "Earliest date of records at GBIF" of one specie. 
```{r}
ggplot() +
  geom_raster(data = df1, aes(x = x, y = y, fill = Value)) + # Use alpha for transparency
  geom_sf(data = borders_eu, fill = NA, color = "gray") +
  scale_fill_viridis_c(name="Year") + # Optional: for better color scale
  theme_minimal() +
  labs(
...
ggplot() +
  ...
  labs(
    ...
    fill = "Year")
```
#### **Metrics 4 and 5:** Basis of records of the earliest and latest date of occurrenc in GBIF.

First, we identify what are the unique "basis of record" of our IAS occurrence cube and add an identifier.
```{r}
...
```
Now we use a data frame obtained previously when computing metric 2 and 3. Specifically, we refer to the second element of funtion `min_max_dates` that is a table with earliest and latest dates of species occurrences at GBIF.
...
```{r}
writeRaster(total_occ_sps, here("output/datacubes/tif_metrics/ias_total_occurrences.tif"), datatype = 'INT4U', overwrite = TRUE)
...
writeRaster(min_dates_datasource, here("output/datacubes/tif_metrics/ias_earliest_date_records_sourceofrecord.tif"), datatype = 'INT4U', overwrite = TRUE)
writeRaster(max_dates_datasource, here("output/datacubes/tif_metrics/ias_latest_date_records_sourceofrecord.tif"), datatype = 'INT4U', overwrite = TRUE)
```

#### Prepare taxonomy for the EBV Data Portal
Due to missing data for 10 species, we prepared a new file including only taxonomic information for our results.
```{r}
