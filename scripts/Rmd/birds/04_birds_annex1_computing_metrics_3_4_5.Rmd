...
subtitle: <span style="color:#47A52A"> Calculation of Metrics for the Birds Directive Annex I Using Occurrence Cubes (Part II)
author: 
...
In this notebook, we calculate simple metrics for the EU Annex I Birds Directive based on species occurrence in GBIF. For this, a bird occurrence cube (see Notebook 01 in this repository) was previously created using the [occurrence cube software](https://techdocs.gbif.org/en/data-use/data-cubes) developed by GBIF under the [Biodiversity Building Blocks for Policy](https://b-cubed.eu/) (B3) Project. Details of the data query in GBIF are available at [doi: 10.15468/dl.uh84tp](https://doi.org/10.15468/dl.uh84tp/). In this notebook we calculate metrics 4, 5 and 6 listed below:
 ...
 - Month with the highest number of records
 - Month with the second highest number of records
 - Month with the third highest number of records
...
#### **Metrics 4, 5 and 6:**  Months with the highest, second-highest, and third-highest number of GBIF records
...
ggplot() +
  ...
  theme_minimal() +
  labs(
    title = bquote("Month with the highest GBIF records for " * italic(.(spi_name$scientificName) * "per pixel")),
    x = "Latitude",
    y = "Longitude",    
    fill = "Month")
```
...
ggplot() +
  ...
  theme_minimal() +
  labs(
    title = bquote("Month with the second-highest GBIF records for " * italic(.(spi_name$scientificName) * "per pixel")),
    x = "Latitude",
    y = "Longitude",    
    fill = "Month")
```
...
```{r}
ggplot() +
  geom_raster(data = df3, aes(x = x, y = y, fill = Value)) + # Use alpha for transparency
  geom_sf(data = borders_eu, fill = NA, color = "gray") +
  scale_fill_viridis(name="Month", option = "C", direction = 1) + # 
  theme_minimal() +
  labs(
    title = bquote("Month with the third-highest GBIF records for " * italic(.(spi_name$scientificName) * "per pixel")),
    x = "Latitude",
    y = "Longitude",    
    fill = "Month")
```
