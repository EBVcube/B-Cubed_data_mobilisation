---
title: <span style="color:#005857"> Data mobilisation from GBIF to the EBV Data Portal<span>
subtitle: <span style="color:#47A52A"> Calculation of Metrics for the Birds Directive Annex I Using Occurrence Cubes (Part I)
author: 
...
ggplot() +
  ...
  # scale_fill_binned(name = "Total occurrences", type = "gradient") + # Optional: for better color scale
  scale_fill_distiller(name="Total occurrences", palette="RdPu") +
  theme_minimal() +
  labs(
    title = bquote("Total number of occurrences of " * italic(.(spi_name$scientificName)) * "at GBIF pixelwise"),
    x = "Latitude",
    y = "Longitude",    
    fill = "Total <br> occurrences")
```

#### **Metric 2 and 3:** Earliest and Latest Month of Species Occurrence at GBIF

We calculate the earliest (latest) month of occurrence for each species. The function `min_max_month` finds the earliest (latest) month of records per species at the pixel level across the entire dataset.
...
ggplot() +
  ...
  theme_minimal() +
  labs(
    title = bquote("Earliest month of GBIF records for " * italic(.(spi_name$scientificName) * " by pixel")),
    x = "Latitude",
    y = "Longitude",    
    fill = "Month")
```
...
ggplot() +
  ...
  theme_minimal() +
  labs(
    title = bquote("Latest month of GBIF records for " * italic(.(spi_name$scientificName) * "by pixel")),
    x = "Latitude",
    y = "Longitude",    
    fill = "Month")
```
